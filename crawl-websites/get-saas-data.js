...
const listOfSaas = require("../listOfSassUrls.json");
...
(async () => {
  ...
  async.eachLimit(listOfSaas, 7, getSaasText, async function (err, res) {
...
async function getSaasText(saasUrl) {
  const pathToFile = path.resolve(__dirname, "saas-data", saasUrl);
  ...
  try {
    ...
    await writeFileAsync(
      path.resolve(__dirname, "saas-data", saasUrl),
      saasText,
      {
        encoding: "utf-8",
        flag: "w",
