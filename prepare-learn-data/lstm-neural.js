...
async.parallel([workOnSaas, workOnNoSaas], function () {
  ...
  const datasNoSaas = preparedForTrainingData.filter(el=>el.output.NOSAAS === 1).slice(350);
  train([...datasSaas, ...datasNoSaas]);  
  fs.writeFileSync(path.resolve(__dirname, 'brain/lstm.json'), JSON.stringify(trainedNet), {encoding: 'utf-8'})
...
function train(data) {
  let net = new brain.recurrent.LSTM({hiddenLayers:[40,10,4]});
  net.train(data, {
    // Defaults values --> expected validation
    iterations: 2500, // the maximum times to iterate the training data --> number greater than 0
